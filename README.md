...
<sub>Screenshots are refreshed as a batch before v1; the Svelte PWA's flows are exercised end to end on every change by the Playwright suite (`scripts/e2e.sh`).</sub>
...
[Svelte web app](docs/decisions/0024-pwa-replaces-liveview.md) at the site
root on the same domain — no extra configuration or separate deployment.
