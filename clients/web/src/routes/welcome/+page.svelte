<script lang="ts">
	...
	// and the sign-in button, never an error message. So the page has
	// exactly two visual states — skeleton while loading, then content —
	// not the three-way loading/ready/error of the authenticated list
	// pages, which is why this tracks a plain `loading` boolean.
	// Two deliberate deltas from the LiveView, both for want of a PWA
	// equivalent: no "Create an account" link (there is no standalone
	// register route) and no per-community avatar-initials tile (no
	// community-avatar component exists).
	...
	onMount(async () => {
		try {
			...
			// A failed fetch degrades silently to ethos + sign-in (see above).
		} finally {
			loading = false;
		}
	});
