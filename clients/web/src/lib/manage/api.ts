 ...
 * the controls, but the server is the enforcer — these calls throw the
 * shared `ApiError` (#270), so a `forbidden` from a stale capability list
 * degrades gracefully, and a page pairing a feed-family fetch
 * (`fetchCommunity`/`fetchGroup`) with a manage call collapses both
 * through one `errorKind`.
 */
